...
export default async function cidadeRouter(fastify: FastifyInstance) {
    ...
    const repository = diContainer.resolve("cidadeRepository");
    ...
    fastify.get<{ Params: { id: string } }>('/:id', async (request, reply) => {
        ...
        const cidade = await repository.get(id);
    ...
    fastify.post<{ Body: Omit<Cidade, 'id'> }>('/', async (request, reply) => {
        ...
        const cidade = await repository.create({nome, estado});
    ...
    fastify.put<{ Params: { id: string }; Body: Partial<Omit<Cidade, 'id'>> }>('/:id', async (request, reply) => {
        const id = parseInt(request.params.id);
        const result = await repository.update(id, {...request.body});
        switch (result.kind) {
